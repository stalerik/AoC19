...
count = 0
cordinates = []
#print(run_int_code([0,1], cp.copy(val), {0:0}))
last_x = 0
answer = 0
for y in range(500,5000):
	saved_count = 0
	found_beam = False
	for x in range(last_x,5000):
		#print(x,y)
		cordinates = [x,y]
		res = run_int_code(cordinates, cp.copy(val), {0:0})
		#print(res)
		if res == 1:
			if not found_beam:
				last_x = x
			found_beam = True
			if run_int_code([x+99,y], cp.copy(val), {0:0}) == 1 and run_int_code([x,y+99],cp.copy(val),{0:0}) == 1:
				answer = x*10000+y
				print(x,y)
				print(answer)
				break
